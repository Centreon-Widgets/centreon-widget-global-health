...
if(isset($preferences['hosts_services']) && $preferences['hosts_services'] == 'hosts'){
    ...
    if(!empty($preferences['hostgroup'])){
        ...
        $hostGroupName = $row['hg_name'];
    }

    if($hostGroupName){
        $innerJoinGroup = ' INNER JOIN hosts_hostgroups hhg ON  h.host_id = hhg.host_id ' .
                           ' INNER JOIN hostgroups hg ON hhg.hostgroup_id = hg.hostgroup_id and hg.name = \'' . $hostGroupName . '\' ';
    }
    ...
    if($serviceGroupName){
        $innerJoinGroup = ' INNER JOIN services_servicegroups ssg ON ssg.service_id = s.service_id and ssg.host_id = s.host_id ' .
                           ' INNER JOIN servicegroups sg ON ssg.servicegroup_id = sg.servicegroup_id and sg.name = \'' . $serviceGroupName . '\' ';
    }
	...
	$groupListString = $oreon->user->access->getAccessGroupsString();

	/**
	 * Get DB information for creating Flash
	 */
	if ($is_admin) {
        ...
        $rq2 = 	' SELECT count(DISTINCT s.state, s.host_id, s.service_id) count, s.state state, ' .
            ...
            ' AND h.enabled = 1 ' .
            ' AND s.enabled = 1 ' .
            ' AND acl.group_id IN (' . $groupListString . ') '.
            ' GROUP BY s.state ORDER by s.state';
	}
